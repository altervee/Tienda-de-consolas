...
$db = new Database();
$con = $db->conectar();
$sql = $con->prepare("SELECT id, nombre, precio FROM productos WHERE activo=1");// LO DE ACTIVO SIRVE PARA QUITAR PRODUCTOS PONIEND EN LA BD SI CAMBIAMOS DE VALOR NO LOS MOSTRARÁ
$sql->execute();// EJECUTAMOS LA CONSULTA
$resulatdo = $sql->fetchAll(PDO::FETCH_ASSOC);//ASOCIAMOS MEDIANTE EL NOMBRE DE CADA COLUMNA
...
<main>
    <div class="container">
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3"><!--Se agregan columnas que se ajustan a la apantalla-->
    <?php foreach($resulatdo as $row){?>
        <div class="col"><!--Visualizar el articulo-->
        <div class="card shadow-sm">
            <?php 
            $id = $row['id'];//Obtenemos la variable id de la BD
            $imagen ="../img/productos/". $id ."/principal.jpg";
            if(!file_exists($imagen)){
                $imagen = "../img/no-photo.jpg";// cuando no hay foto existente o esta mal se añade una predefinida
            }
            ?>
            <img alt="" src="<?php echo $imagen;?>" class="d-block w-100">
            <div class="card-body"><!--cuerpo del articulo-->
            <h5 class="card-title"><?php echo $row['nombre'];?></h5><!--titulo del articulo-->
            <p class="card-text">€ <?php echo number_format($row['precio'], 2, '.', ','); ?></p>
            <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                    <a href="details.php?id=<?php echo $row['id']; ?>&token=<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN);?>" class="btn btn-primary">Detalles</a>
                </div><!--hash_hmac('sha1', $row['id'], KEY_TOKEN) ciframos la ID CON NUESTRA CONTRASEÑA-->
                <button class="btn btn-outline-success" 
                type="button" onclick="addProducto(<?php echo $row['id'];?>, '<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN);?>')">Agregar al carrito</button>
